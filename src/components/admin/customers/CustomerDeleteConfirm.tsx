...
// Interface cho dialog xác nhận xóa khách hàng
interface Customer {
    id: number;
    fullName: string;
    email: string;
...
export default function CustomerDeleteConfirm({ open, setOpen, customer, refresh }: DeleteConfirmProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                showError(data.error || "Có lỗi xảy ra khi xóa khách hàng");
            }
        } catch (error) {
                ...
                <div className="p-6 space-y-4">
                    <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded">
                        <p className="text-gray-700">
                            ...
                            <span className="font-bold text-red-600"> {customer.fullName}</span>?
                        </p>
                    ...
                    <p className="text-sm text-gray-600">
                        ⚠️ Không thể xóa nếu khách hàng đã có đơn hàng!
                    </p>
                </div>
